...
def jointCreation(prefix, chainTypes, name, position):
    for n in name:
        ...
        cmds.joint(n = prefix + '_' + chainTypes + '_' + n + '_jnt', p = position[name.index(n)])
        if name.index(n) != 0:
...
rig_info['fkJnts'] = jointCreation('c', 'FK', ['shoulder', 'elbow', 'wrist', 'endWrist'], [[0, 0, 0], [7, 0, -1], [14, 0, 0], [15, 0, 0]])

rig_info['rigJnts'] = jointCreation('c', 'RIG', ['shoulder', 'elbow', 'wrist', 'endWrist'], [[0, 0, 0], [7, 0, -1], [14, 0, 0], [15, 0, 0]])

for key, value in rig_info.iteritems():
    print (key, value)
